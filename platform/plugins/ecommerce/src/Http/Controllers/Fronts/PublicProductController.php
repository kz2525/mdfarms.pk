...
class PublicProductController
{
    ...
    public function getProductVariation($id, Request $request, BaseHttpResponse $response)
    {
        ...
        if ($attributes = $request->input('attributes', [])) {
            ...
            $attributes = $product->defaultVariation->productAttributes->pluck('id')->all();
        }
        ...
        if ($product) {
            if ($product->images) {
                ...
                if (get_ecommerce_setting('how_to_display_product_variation_images') == 'variation_images_and_main_product_images') {
                    $originalImages = array_merge($originalImages, json_decode($product->original_images));
                }
            } else {
                $originalImages = json_decode($product->original_images ?: $product->original_product->images);
            }
