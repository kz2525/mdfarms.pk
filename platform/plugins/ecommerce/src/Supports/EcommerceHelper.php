...
class EcommerceHelper
{
    ...
    {
        return (bool)get_ecommerce_setting('make_phone_field_at_the_checkout_optional', 0);
    }
    ...
    public function getProductVariationInfo(Product $product, array $params = []): array
    {
        ...
        if ($product->variations->count()) {
            ...
            if ($params) {
                $product->loadMissing(['variations.productAttributes', 'variations.productAttributes.productAttributeSet']);
                $variations = collect();
                foreach ($params as $key => $value) {
                    $product->variations->map(function ($variation) use ($value, $key, &$variations) {
                        $productAttribute = $variation->productAttributes->filter(function ($attribute) use ($value, $key) {
                            return $attribute->slug == $value && $attribute->productAttributeSet->slug == $key;
                        })->first();

                        if ($productAttribute && ! $variations->firstWhere('id', $productAttribute->id)) {
